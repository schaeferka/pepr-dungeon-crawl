...
void monsterPodAction(creature *monst, char action[100])
{

    int monstDepth = rogue.depthLevel;

    char command[256];

    // Construct the command to invoke the JavaScript utility
    snprintf(command, sizeof(command), "node --no-deprecation ./client/deploymentAction.js %i \"%s\" %s %i\n", monst->monsterDeploymentID, monst->info.monsterName, action, monstDepth);
    ...
    if (result != 0)
    {
        fprintf(stderr, "Finished processing action on: \"%s\"\n", monst->monsterDeploymentName);
        // fprintf(stderr, "node ./k8s/deploymentAction.js %i \"%s\" %s %i\n", monst->monsterDeploymentID, monst->info.monsterName, action, monstDepth);
        // return 1; // Return an error code
    }
    else
    {

        // printf("Requested action on pod successfully initiated\n");
        // return 0;
    }
}
...
boolean monsterHiddenBySubmersion(const creature *monst, const creature *observer)
{
    ...
    {
        if (observer && (terrainFlags(observer->loc.x, observer->loc.y) & T_IS_DEEP_WATER) && !observer->status[STATUS_LEVITATING])
        {
...
{
    if (cellHasTerrainFlag(defender->loc.x, defender->loc.y, T_OBSTRUCTS_PASSABILITY) && !(defender->info.flags & MONST_ATTACKABLE_THRU_WALLS))
    {
        return true;
    }
    if (attacker == &player)
...
boolean monstersAreEnemies(const creature *monst1, const creature *monst2)
{
    ...
    if (((monst1->info.flags & MONST_RESTRICTED_TO_LIQUID) && !(monst2->info.flags & MONST_IMMUNE_TO_WATER) && !(monst2->status[STATUS_LEVITATING]) && cellHasTerrainFlag(monst2->loc.x, monst2->loc.y, T_IS_DEEP_WATER))

        || ((monst2->info.flags & MONST_RESTRICTED_TO_LIQUID) && !(monst1->info.flags & MONST_IMMUNE_TO_WATER) && !(monst1->status[STATUS_LEVITATING]) && cellHasTerrainFlag(monst1->loc.x, monst1->loc.y, T_IS_DEEP_WATER)))
    {
...
{
    return ((monst->info.flags & MONST_SUBMERGES) && cellHasTMFlag(monst->loc.x, monst->loc.y, TM_ALLOWS_SUBMERGING) && !cellHasTerrainFlag(monst->loc.x, monst->loc.y, T_OBSTRUCTS_PASSABILITY) && !(monst->bookkeepingFlags & (MB_SEIZING | MB_SEIZED | MB_CAPTIVE)) && ((monst->info.flags & (MONST_IMMUNE_TO_FIRE | MONST_INVULNERABLE)) || monst->status[STATUS_IMMUNE_TO_FIRE] || !cellHasTerrainFlag(monst->loc.x, monst->loc.y, T_LAVA_INSTA_DEATH)));
}
...
creature *spawnHorde(short hordeID, pos loc, unsigned long forbiddenFlags, unsigned long requiredFlags)
{
    ...
    if (hordeID <= 0)
    {
        ...
        do
        {
            ...
            if (isPosInMap(loc))
            {
                if (cellHasTerrainFlag(loc.x, loc.y, T_PATHING_BLOCKER) && (!hordeCatalog[hordeID].spawnsIn || !cellHasTerrainType(loc.x, loc.y, hordeCatalog[hordeID].spawnsIn)))
                {
...
void prependCreature(creatureList *list, creature *add)
{
    ...
    list->head = node;
    MONSTER_COUNT++;
    add->monsterDeploymentID = MONSTER_COUNT;

    // Assuming MONSTER_COUNT is an integer, calculate the length of the monsterDeploymentID as string
    int idLength = snprintf(NULL, 0, "%d", add->monsterDeploymentID);
    // Calculate total length needed: type length + 1 for '-' + idLength + 1 for '\0'
    int totalLength = strlen(add->info.monsterName) + 1 + idLength + 1;

    // Allocate memory for monsterDeploymentName
    add->monsterDeploymentName = calloc(totalLength, sizeof(char));
    if (add->monsterDeploymentName != NULL)
    {
        // Format and store the combined string
        sprintf(add->monsterDeploymentName, "%s-%d", add->info.monsterName, add->monsterDeploymentID);
    }
    else
    {
        // Handle memory allocation failure
        fprintf(stderr, "Failed to allocate memory for monsterDeploymentName\n");
    }
    fprintf(stderr, "Prepending creature: %s\n", add->monsterDeploymentName);
    // add->info.monsterName = add->monsterDeploymentName;
    monsterPodAction(add, "create");
}
    ...
    while (*node != NULL)
    {
        if ((*node)->creature == remove)
        {
            ...
            *node = removeNode->nextCreature;
            free(removeNode);
            monsterPodAction(remove, "delete");
            return true;
        }
...
creature *findDecedent(const char *creatureName) {
    fprintf(stdout, "Searching for decedent by deployment name: %s\n", creatureName);
    boolean creatureFound = false;
    while (!creatureFound) {
        for (int i = 0; i < gameConst->deepestLevel+1; ++i) {
            // fprintf(stdout, "Searching for creature on level with findDecedent: %d\n", i);
            if (levels[i].monsters.head == NULL)
            {
                // fprintf(stdout, "No creatures on this level\n");
                continue;
            }
            creatureList *list = &levels[i].monsters;
            creatureListNode *node = list->head;

            while (node != NULL) {
                if (node->creature != NULL && node->creature->monsterDeploymentName != NULL) {
                    // fprintf(stdout, "Checking creature: %s\n", node->creature->monsterDeploymentName);
                    if (strcmp(node->creature->monsterDeploymentName, creatureName) == 0) {
                        fprintf(stdout, "Woohoo! Found creature with deployment name: %s\n", node->creature->monsterDeploymentName);
                        creatureFound = true;
                        // Return the creature pointer directly
    ...
    return NULL; // Creature not found
}

boolean findCreature(const char *creatureName) {
    // fprintf(stdout, "Checking to see if creature exists: %s\n", creatureName);
    boolean creatureFound = false;
    while (!creatureFound) {
        for (int i = 0; i < gameConst->deepestLevel+1; ++i) {
            // fprintf(stdout, "Searching for creature on level: %d\n", i);
            if (levels[i].monsters.head == NULL)
            {
                // fprintf(stdout, "No creatures on this level\n");
                continue;
...
boolean monsterAvoids(creature *monst, pos p)
{
    ...
    // dry land
    if (monst->info.flags & MONST_RESTRICTED_TO_LIQUID && !cellHasTMFlag(p.x, p.y, TM_ALLOWS_SUBMERGING))
    {
        return true;
    }
    ...
    {
        if (monst != &player && cellHasTMFlag(p.x, p.y, TM_IS_SECRET) && !(discoveredTerrainFlagsAtLoc(p) & avoidedFlagsForMonster(&(monst->info))))
        {
    ...
    // brimstone
    if (!(monst->status[STATUS_IMMUNE_TO_FIRE]) && !(monst->info.flags & MONST_INVULNERABLE) && (tFlags & T_SPONTANEOUSLY_IGNITES) && !(cFlags & (HAS_MONSTER | HAS_PLAYER)) && !cellHasTerrainFlag(monst->loc.x, monst->loc.y, T_IS_FIRE | T_SPONTANEOUSLY_IGNITES) && (monst == &player || (monst->creatureState != MONSTER_TRACKING_SCENT && monst->creatureState != MONSTER_FLEEING)))
    {
        return true;
    }
    ...
    // fire
    if ((tFlags & T_IS_FIRE & ~terrainImmunities) && !cellHasTerrainFlag(monst->loc.x, monst->loc.y, T_IS_FIRE) && !(cFlags & (HAS_MONSTER | HAS_PLAYER)) && (monst != &player || rogue.mapToShore[p.x][p.y] >= player.status[STATUS_IMMUNE_TO_FIRE]))
    {
        return true;
    }

    // non-fire harmful terrain
    if ((tFlags & T_HARMFUL_TERRAIN & ~T_IS_FIRE & ~terrainImmunities) && !cellHasTerrainFlag(monst->loc.x, monst->loc.y, (T_HARMFUL_TERRAIN & ~T_IS_FIRE)))
    {
        return true;
    }
    ...
    if ((tFlags & T_IS_DF_TRAP & ~terrainImmunities) && !(cFlags & PRESSURE_PLATE_DEPRESSED) && (monst == &player || monst->creatureState == MONSTER_WANDERING || (monst->creatureState == MONSTER_ALLY && !(cellHasTMFlag(p.x, p.y, TM_IS_SECRET)))) && !(monst->status[STATUS_ENTRANCED]) && (!(tFlags & T_ENTANGLES) || !(monst->info.flags & MONST_IMMUNE_TO_WEBS)))
    {
        return true;
    }
    ...
    // deep water
    if ((tFlags & T_IS_DEEP_WATER & ~terrainImmunities) && (!(tFlags & T_ENTANGLES) || !(monst->info.flags & MONST_IMMUNE_TO_WEBS)) && !cellHasTerrainFlag(monst->loc.x, monst->loc.y, T_IS_DEEP_WATER))
    {
        return true; // avoid only if not already in it
    }

    // poisonous lichen
    if ((tFlags & T_CAUSES_POISON & ~terrainImmunities) && !cellHasTerrainFlag(monst->loc.x, monst->loc.y, T_CAUSES_POISON) && (monst == &player || monst->creatureState != MONSTER_TRACKING_SCENT || monst->currentHP < 10))
    {
        return true;
    }

    // Smart monsters don't attack in corridors if they belong to a group and they can help it.
    if ((monst->info.abilityFlags & MA_AVOID_CORRIDORS) && !(monst->status[STATUS_ENRAGED] && monst->currentHP <= monst->info.maxHP / 2) && monst->creatureState == MONSTER_TRACKING_SCENT && (monst->bookkeepingFlags & (MB_FOLLOWER | MB_LEADER)) && passableArcCount(p.x, p.y) >= 2 && passableArcCount(monst->loc.x, monst->loc.y) < 2 && !cellHasTerrainFlag(monst->loc.x, monst->loc.y, (T_HARMFUL_TERRAIN & ~terrainImmunities)))
    {
        return true;
    }
...
short awarenessDistance(creature *observer, creature *target)
{
    ...
    if ((target == &player && (pmapAt(observer->loc)->flags & IN_FIELD_OF_VIEW)) || (target != &player && openPathBetween(observer->loc.x, observer->loc.y, target->loc.x, target->loc.y)))
    {
...
short closestWaypointIndexTo(pos p)
{
    short i, closestDistance, closestIndex;
...
void wanderToward(creature *monst, pos destination)
{
    if (isPosInMap(destination))
    {
...
void updateMonsterState(creature *monst)
{
    ...
    for (creatureIterator it = iterateCreatures(monsters); !handledPlayer || hasNextCreature(it);)
    {
        ...
        handledPlayer = true;
        if (monsterFleesFrom(monst, monst2) && distanceBetween((pos){x, y}, monst2->loc) < closestFearedEnemy && traversiblePathBetween(monst2, x, y) && openPathBetween(x, y, monst2->loc.x, monst2->loc.y))
        {
    ...
    for (i = 0; i < 40; i++)
    {
        ...
        if (((blinkUphill && (nowPreference > bestPreference)) || (!blinkUphill && (nowPreference < bestPreference))) && !monsterAvoids(monst, impact))
        {
            ...
            if ((abs(impact.x - origin.x) > 1 || abs(impact.y - origin.y) > 1)
                ...
                || (cellHasTerrainFlag(impact.x, origin.y, T_OBSTRUCTS_PASSABILITY)) || (cellHasTerrainFlag(origin.x, impact.y, T_OBSTRUCTS_PASSABILITY)))
            {
                gotOne = true;
            }
            else
...
boolean isLocalScentMaximum(pos loc)
{
    ...
    {
        pos newLoc = posNeighborInDirection(loc, dir);
        if (isPosInMap(newLoc) && (scentMap[newLoc.x][newLoc.y] > baselineScent) && !cellHasTerrainFlag(newLoc.x, newLoc.y, T_OBSTRUCTS_PASSABILITY) && !diagonalBlocked(loc.x, loc.y, newLoc.x, newLoc.y, false))
        {
...
enum directions scentDirection(creature *monst)
{
    ...
    for (;;)
    {
        ...
        for (dir = 0; dir < DIRECTION_COUNT; dir++)
        {
            ...
            if (coordinatesAreInMap(newX, newY) && (scentMap[newX][newY] > bestNearbyScent) && (!(pmap[newX][newY].flags & HAS_MONSTER) || (otherMonst && canPass(monst, otherMonst))) && !cellHasTerrainFlag(newX, newY, T_OBSTRUCTS_PASSABILITY) && !diagonalBlocked(x, y, newX, newY, false) && !monsterAvoids(monst, (pos){newX, newY}))
            {
...
void moveAlly(creature *monst)
{
    ...
    if (cellHasTerrainFlag(x, y, (T_HARMFUL_TERRAIN & ~(T_IS_FIRE | T_CAUSES_DAMAGE | T_CAUSES_PARALYSIS | T_CAUSES_CONFUSION))) || (cellHasTerrainFlag(x, y, T_IS_FIRE) && !monst->status[STATUS_IMMUNE_TO_FIRE]) || (cellHasTerrainFlag(x, y, T_CAUSES_DAMAGE | T_CAUSES_PARALYSIS | T_CAUSES_CONFUSION) && !(monst->info.flags & (MONST_INANIMATE | MONST_INVULNERABLE))))
    {
    ...
    {
        creature *target = nextCreature(&it);
        if (target != monst && (!(target->bookkeepingFlags & MB_SUBMERGED) || (monst->bookkeepingFlags & MB_SUBMERGED)) && monsterWillAttackTarget(monst, target) && distanceBetween((pos){x, y}, target->loc) < shortestDistance && traversiblePathBetween(monst, target->loc.x, target->loc.y) && (!cellHasTerrainFlag(target->loc.x, target->loc.y, T_OBSTRUCTS_PASSABILITY) || (target->info.flags & MONST_ATTACKABLE_THRU_WALLS)) && (!target->status[STATUS_INVISIBLE] || rand_percent(33)))
        {
...
void monstersTurn(creature *monst)
{
             ...
             || ((monst->info.flags & MONST_RESTRICTED_TO_LIQUID) && !cellHasTMFlag(player.loc.x, player.loc.y, TM_ALLOWS_SUBMERGING)))
    {
        ...
        // if we're standing in harmful terrain and there is a way to escape it, spend this turn escaping it.
        if (cellHasTerrainFlag(x, y, (T_HARMFUL_TERRAIN & ~T_IS_FIRE)) || (cellHasTerrainFlag(x, y, T_IS_FIRE) && !monst->status[STATUS_IMMUNE_TO_FIRE] && !(monst->info.flags & MONST_INVULNERABLE)))
        {
            if (!rogue.updatedMapToSafeTerrainThisTurn)
            {
                updateSafeTerrainMap();
...
{
    return (!cellHasTerrainFlag(loc.x, loc.y, T_OBSTRUCTS_PASSABILITY) || (cellHasTMFlag(loc.x, loc.y, TM_IS_SECRET) && !(discoveredTerrainFlagsAtLoc(loc) & T_OBSTRUCTS_PASSABILITY)));
}
...
void setMonsterLocation(creature *monst, pos newLoc)
{
    ...
    if (playerCanSee(newLoc.x, newLoc.y) && cellHasTMFlag(newLoc.x, newLoc.y, TM_IS_SECRET) && cellHasTerrainFlag(newLoc.x, newLoc.y, T_OBSTRUCTS_PASSABILITY))
    {
...
boolean moveMonster(creature *monst, short dx, short dy)
{
    ...
    // Caught in spiderweb?
    if (monst->status[STATUS_STUCK] && !(pmap[newX][newY].flags & (HAS_PLAYER | HAS_MONSTER)) && cellHasTerrainFlag(x, y, T_ENTANGLES) && !(monst->info.flags & MONST_IMMUNE_TO_WEBS))
    {
    ...
    for (maxPermissibleDifference = 1; maxPermissibleDifference < max(DCOLS, DROWS); maxPermissibleDifference++)
    {
        for (i = 0; i < DCOLS; i++)
        {
            for (j = 0; j < DROWS; j++)
            {
                ...
                if (dist <= maxPermissibleDifference && dist > 0 && !(pmap[sCols[i]][sRows[j]].flags & (HAS_PLAYER | HAS_MONSTER)) && !monsterAvoids(monst, (pos){sCols[i], sRows[j]}) && !(monst == &player && cellHasTerrainFlag(sCols[i], sRows[j], T_PATHING_BLOCKER)))
                {
                             ...
                             boolean deterministic)
{
    ...
    for (int k = 0; k < max(DROWS, DCOLS) && !candidateLocs; k++)
    {
        for (int i = target.x - k; i <= target.x + k; i++)
        {
            ...
            {
                if (coordinatesAreInMap(i, j) && (i == target.x - k || i == target.x + k || j == target.y - k || j == target.y + k) && (!blockingMap || !blockingMap[i][j]) && !cellHasTerrainFlag(i, j, forbiddenTerrainFlags) && !(pmap[i][j].flags & forbiddenMapFlags) && (!forbidLiquid || pmap[i][j].layers[LIQUID] == NOTHING) && (hallwaysAllowed || passableArcCount(i, j) < 2))
                {
                    candidateLocs++;
                }
            }
