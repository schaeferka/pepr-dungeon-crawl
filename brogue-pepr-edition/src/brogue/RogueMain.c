...
void createK8sNamespaces(void) {
    // Command to execute the Node.js script
    // If the script is not in the current directory, specify the full path
    const char* command = "node --no-deprecation ./client/createNamespaces.js";

    printf("Creating Kubernetes namespaces...\n");

    // Execute the command
    int result = system(command);

    if (result != 0) {
        fprintf(stderr, "Failed to execute command to create namespaces\n");
    } else {
        printf("Kubernetes namespaces created successfully\n");
    }
}

void deleteK8sNamespaces(void) {
    // Command to execute the Node.js script
    // If the script is not in the current directory, specify the full path
    const char* command = "node --no-deprecation ./client/deleteNamespaces.js";

    printf("Deleting Kubernetes namespaces...\n");

    // Execute the command
    int result = system(command);

    if (result != 0) {
        fprintf(stderr, "Failed to execute command to delete namespaces\n");
    } else {
        printf("Kubernetes namespaces deleted successfully\n");
    }
}
