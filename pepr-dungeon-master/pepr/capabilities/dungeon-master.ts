...
When(a.Deployment)
  ...
  .Validate(req => {
    ...
    if (monsterType == "pink-jelly") {
      ...
      notifyDeletion(monstie, "dungeon-master")
        .then(() => {
...
When(a.Deployment)
  ...
  .Validate(req => {
    ...
    try {
      ...
      Log.info(`Delete type: ${deleteType}`);
    } catch (error) {
      deleteType = req.Raw.metadata?.labels?.["deleteType"];
    ...
    if (deleteType == "killed") {
      ...
      notifyDeletion(monstie, deleteType)
        .then(() => {
    ...
    Log.info(`Default process is deleting ${monstie} from the game.`);
